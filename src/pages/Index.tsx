...
import { getProductsFromCollection, subscribeToNewsletter } from "@/lib/shopify";
...
const Index = () => {
  ...
  const {
    ...
    queryKey: ['products'],
    queryFn: () => getProductsFromCollection('frontpage', 4)
  });
      ...
      <div className="container relative z-10 py-20 md:py-32">
        <div className="flex justify-end">
          <div className="max-w-2xl space-y-8 animate-fade-in text-right">
            ...
            {/* Social proof */}
            <div className="flex flex-col sm:flex-row items-center gap-4 pt-6 justify-end">
              <div className="flex -space-x-3">
                {[testimonialGirl1, testimonialGirl2, testimonialGirl3, testimonialGirl4].map((img, i) => (
                  <img
                    ...
                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-3 sm:border-4 border-white shadow-xl"
                  />
                ))}
              </div>
              <div className="text-sm bg-white/95 backdrop-blur-sm px-4 py-2 rounded-xl shadow-lg">
                <div className="flex text-yellow-500 mb-1 justify-center sm:justify-start">
                  {[1, 2, 3, 4, 5].map(i => <Star key={i} className="w-4 h-4 fill-current" />)}
                </div>
                <p className="text-brand-purple font-bold text-center sm:text-left">4.9/5 from 2,500+ reviews</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            },
            {
              ...
              label: "Customer Love",
              description: "10,000+ happy customers and growing, with repeat orders every month.",
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-14">
            ...
            <div className="flex-1 w-full space-y-4">
              ...
              "
                ...
                "
                  >
                    <span className="relative">{isSubscribing ? 'Subscribing...' : 'Subscribe'}</span>
                  </Button>
                </form>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 justify-between text-xs md:text-sm font-nunito text-foreground/70">
                <div className="flex items-center gap-3">
                  <div className="flex -space-x-1 flex-shrink-0">
                    {[testimonialGirl1, testimonialGirl2, testimonialGirl3].map((img, i) => (
                      <img
                        key={i}
                        src={img}
                        alt={`Happy customer ${i + 1}`}
                        className="w-6 h-6 rounded-full object-cover border-2 border-white shadow-sm flex-shrink-0"
                      />
                    ))}
                  </div>
                  <span>
